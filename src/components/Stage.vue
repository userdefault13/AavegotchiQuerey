<template>
  <div class="stage-container">
    ...
    <div v-else-if="gotchiData" class="stage-content">
      ...
      <div v-if="svgViews" class="flex justify-end">
        <button 
      ...
      <div v-if="isDressingRoomMode && svgViews" class="dressing-room-section">
        <div class="panel-header">
          <h3>Dressing Room</h3>
          <button 
            @click="isDressingRoomCollapsed = !isDressingRoomCollapsed" 
            class="collapse-btn"
            :title="isDressingRoomCollapsed ? 'Expand' : 'Collapse'"
          >
            {{ isDressingRoomCollapsed ? '▼' : '▲' }}
          </button>
        </div>
        
        <div v-if="!isDressingRoomCollapsed" class="dressing-room-content">
          <div class="dressing-room-layout">
            ...
            <!-- Controls Panel -->
            <div class="dressing-room-controls">
              ...
              <div class="wearable-browser">
                ...
                <div v-if="isLoadingWearables && loadingProgress" class="loading-progress-container">
                  ...
                  <p class="loading-progress-text">
                    Loading wearables... {{ loadingProgress.loaded || 0 }} / {{ loadingProgress.total || 1000 }} ({{ loadingProgress.percentage || 0 }}%)
                  </p>
                </div>
      ...
      <div class="breakdown-section" v-if="availableViews.length > 0">
        ...
        <div class="tab-content">
          ...
          <div v-else-if="availableViews.length > 0 && svgViews" class="loading-code">
            <p class="loading-text">Loading {{ activeBreakdownTab }} breakdown...</p>
          </div>
          <div v-else class="loading-code">
            <p class="text-gray-500 dark:text-gray-400">Breakdown not available. SVG data may still be loading.</p>
          </div>
        </div>
      </div>
...
const isDressingRoomCollapsed = ref(false)
...
// Extracted wearables from previewSideAavegotchi (when it succeeds)
// Format: { viewName: { slot: { svg: string, element: Element } } }
const extractedWearablesByView = ref({})
...
let retryAttempts = 0 // Track retry attempts for exponential backoff
watch([previewWearables, isDressingRoomMode], async ([wearables, isActive]) => {
  ...
  // Don't generate preview if we're still loading wearables (to avoid too many requests)
  if (isLoadingWearables.value) {
    // console.log('Skipping preview generation - wearables still loading')
    return
  }
  
  // Debounce preview generation to avoid too many contract calls (increased delay)
  if (previewTimeout) {
  ...
  try {
    ...
    try {
      ...
      if (sidesArray && sidesArray.length >= 4) {
        ...
        // Extract wearables from previewSideAavegotchi result after determining view order
        // This gives us correctly positioned wearables directly from the contract
        const extractedWearables = {}
        const viewMap = {
          0: 'Front',
          [leftIndex]: 'Left',
          [rightIndex]: 'Right',
        ...
        // Also extract from Front view (index 0)
        if (sidesArray[0]) {
          const frontWearables = extractWearablesFromPreviewSvg(sidesArray[0], 'Front', gotchiData.value.equippedWearables)
          if (Object.keys(frontWearables).length > 0) {
            extractedWearables['Front'] = frontWearables
          }
        }
        
        sidesArray.forEach((svgString, index) => {
          const viewName = viewMap[index]
          if (viewName && svgString && index !== 0) { // Skip index 0, already handled above
            const wearables = extractWearablesFromPreviewSvg(svgString, viewName, gotchiData.value.equippedWearables)
            if (Object.keys(wearables).length > 0) {
              extractedWearables[viewName] = wearables
            }
          }
        })
        
        // Store extracted wearables
        if (Object.keys(extractedWearables).length > 0) {
          extractedWearablesByView.value = extractedWearables
          console.log('Extracted wearables from previewSideAavegotchi:', extractedWearables)
        }
      ...
      try {
        ...
        // Extract wearables from getAavegotchiSideSvgs result (fallback)
        // This also contains wearables, so we should extract them
        if (sidesArray && sidesArray.length >= 4) {
          const viewNames = ['Front', 'Left', 'Right', 'Back']
          const extractedWearables = {}
          
          // Try to determine left/right order (same logic as below)
          const view1 = (sidesArray[1] || '').toLowerCase()
          const view2 = (sidesArray[2] || '').toLowerCase()
          const view1IsLeft = view1.includes('bodyleft') || view1.includes('gotchi-bodyleft')
          const view2IsLeft = view2.includes('bodyleft') || view2.includes('gotchi-bodyleft')
          ...
          // Extract from Front view (index 0)
          if (sidesArray[0]) {
            const frontWearables = extractWearablesFromPreviewSvg(sidesArray[0], 'Front', gotchiData.value.equippedWearables)
            if (Object.keys(frontWearables).length > 0) {
              extractedWearables['Front'] = frontWearables
          ...
          if (Object.keys(extractedWearables).length > 0) {
            extractedWearablesByView.value = extractedWearables
            console.log('Extracted wearables from getAavegotchiSideSvgs (fallback):', extractedWearables)
          }
        }
...
async function generatePreviewSvgs() {
  ...
  try {
    ...
    // Extract wearables from previewSideAavegotchi result for dressing room preview
    // This gives us correctly positioned wearables for the preview
    if (previewArray.length >= 4) {
      const extractedWearables = {}
      const viewNames = ['Front', 'Left', 'Right', 'Back']
      
      previewArray.forEach((svgString, index) => {
        const viewName = viewNames[index]
        if (viewName && svgString) {
          const wearables = extractWearablesFromPreviewSvg(svgString, viewName, validatedWearablesWithCheck)
          if (Object.keys(wearables).length > 0) {
            extractedWearables[viewName] = wearables
          }
        }
      ...
      // Update extracted wearables for preview (merge with existing)
      if (Object.keys(extractedWearables).length > 0) {
        extractedWearablesByView.value = {
          ...extractedWearablesByView.value,
          ...extractedWearables
        }
        console.log('Extracted wearables from previewSideAavegotchi (preview):', extractedWearables)
      }
    }
    ...
    // Clear any rate limit error if preview generation succeeded
    if (previewError.value && previewError.value.includes('Rate limit exceeded')) {
      previewError.value = null
    }
    ...
    // Handle rate limiting errors (429) - but NOT "SVG type or id does not exist" errors
    if (error.message?.includes('SVG type or id does not exist')) {
      // This is a contract error, not a rate limit - handle it separately below
    } else if (error.message?.includes('429') || error.message?.includes('compute units per second') || error.code === 429) {
      previewError.value = 'Rate limit exceeded: Too many requests to the blockchain. Retrying automatically in a few seconds...'
      ...
      // Exponential backoff: wait longer with each retry attempt
      const retryDelay = Math.min(5000 * Math.pow(2, retryAttempts), 30000) // Start at 5s, max 30s
      retryAttempts++
      
      console.log(`Rate limit hit. Retrying in ${retryDelay}ms (attempt ${retryAttempts})`)
      
      // Automatically retry after delay
      rateLimitRetryTimeout = setTimeout(async () => {
        if (isDressingRoomMode.value && gotchiData.value && !isLoadingWearables.value) {
          console.log('Retrying preview generation after rate limit error...')
          previewError.value = 'Retrying preview generation...'
          await generatePreviewSvgs()
        } else {
          // Reset retry attempts if we can't retry
          retryAttempts = 0
        }
        rateLimitRetryTimeout = null
      }, retryDelay)
      
      return // Exit early - don't process other error types
    }
    
    // Reset retry attempts on non-rate-limit errors
    retryAttempts = 0
    ...
    if (error.message && error.message.includes('SVG type or id does not exist')) {
      // This error means previewSideAavegotchi failed, likely due to hauntId or contract limitation
      // Fall back to building preview manually using buildSvgFromParts
      console.warn('previewSideAavegotchi failed with "SVG type or id does not exist". Falling back to manual preview building.')
      
      try {
        // Build preview manually using buildSvgFromParts for each view
        const manualPreviewSvgs = {}
        const views = ['Front', 'Left', 'Right', 'Back']
        
        for (const viewName of views) {
          const builtSvg = buildSvgFromParts(viewName)
          if (builtSvg) {
            manualPreviewSvgs[viewName] = builtSvg
          }
        }
        
        if (Object.keys(manualPreviewSvgs).length > 0) {
          previewSvgs.value = manualPreviewSvgs
          
          // Apply styles to preview SVGs
          await nextTick()
          Object.values(manualPreviewSvgs).forEach(svg => {
            if (svg) {
              applySvgStyles(svg)
            }
          })
          
          console.log('Built preview manually using buildSvgFromParts:', Object.keys(manualPreviewSvgs))
          previewError.value = null // Clear any error
          return // Success - exit early
        }
      } catch (buildError) {
        console.error('Failed to build preview manually:', buildError)
        // Fall through to show error
      }
      
      // If manual building also failed, show error
...
// Extract wearables from previewSideAavegotchi SVG result
// This extracts wearable elements that are already correctly positioned by the contract
function extractWearablesFromPreviewSvg(svgString, viewName, equippedWearables) {
  ...
  try {
    ...
    // Find all elements with gotchi-wearable class
    const wearableElements = svgElement.querySelectorAll('.gotchi-wearable, [class*="gotchi-wearable"]')
    
    // Also find sleeve elements that might be nested inside wearable groups
    const sleeveElements = svgElement.querySelectorAll('.gotchi-sleeves, [class*="gotchi-sleeves"], [class*="sleeve"]')
    
    // Process regular wearables first
    wearableElements.forEach(element => {
      const classes = (element.getAttribute('class') || '').toLowerCase()
      
      // Skip if this is a sleeve element (we'll process those separately)
      const isSleeve = classes.includes('gotchi-sleeves') || classes.includes('sleeve')
      if (isSleeve) return
      
      // Map class names to slots
      let slot = null
      if (classes.includes('wearable-body')) slot = 0
      else if (classes.includes('wearable-face')) slot = 1
      else if (classes.includes('wearable-eyes')) slot = 2
      else if (classes.includes('wearable-head')) slot = 3
      else if (classes.includes('wearable-hand-left') || (classes.includes('wearable-hand') && classes.includes('left'))) slot = 4
      else if (classes.includes('wearable-hand-right') || (classes.includes('wearable-hand') && !classes.includes('left'))) slot = 5
      else if (classes.includes('wearable-pet')) slot = 6
      else if (classes.includes('wearable-bg')) slot = 7
      
      if (slot !== null) {
        // Regular wearable (not a sleeve)
        // Extract the element with its positioning
        // Check if it's wrapped in an SVG with x/y attributes
        let parentSvg = element.closest('svg[x], svg[y]')
        let wearableSvg = ''
        
        if (parentSvg && (parentSvg.hasAttribute('x') || parentSvg.hasAttribute('y'))) {
          // Element is already positioned in a nested SVG
          // Extract the entire nested SVG structure
          const x = parentSvg.getAttribute('x') || '0'
          const y = parentSvg.getAttribute('y') || '0'
          
          // Get the group containing the wearable
          const wearableGroup = element.closest('g.gotchi-wearable') || element
          wearableSvg = serializer.serializeToString(wearableGroup)
        } else {
          // Element doesn't have explicit positioning, serialize as-is
          wearableSvg = serializer.serializeToString(element)
        }
    ...
    // Process sleeve elements separately
    sleeveElements.forEach(element => {
      const classes = (element.getAttribute('class') || '').toLowerCase()
      
      // Sleeves are always part of body wearable (slot 0)
      // Extract the element with its positioning
      let parentSvg = element.closest('svg[x], svg[y]')
      let sleeveSvg = ''
      ...
      if (parentSvg && (parentSvg.hasAttribute('x') || parentSvg.hasAttribute('y'))) {
        // Element is already positioned in a nested SVG
        // Serialize the sleeve element itself
        sleeveSvg = serializer.serializeToString(element)
        
        // Wrap in the same positioning SVG if needed
        const x = parentSvg.getAttribute('x') || '0'
        const y = parentSvg.getAttribute('y') || '0'
        sleeveSvg = `<g class="gotchi-wearable wearable-sleeves"><svg x="${x}" y="${y}">${sleeveSvg}</svg></g>`
      } else {
        // Element doesn't have explicit positioning, serialize as-is
        sleeveSvg = serializer.serializeToString(element)
        // Still wrap it for consistency
        sleeveSvg = `<g class="gotchi-wearable wearable-sleeves">${sleeveSvg}</g>`
      }
...
// Build SVG from gotchi parts
function buildSvgFromParts(viewName = 'Front') {
  const parts = baseGotchiPartsByView.value[viewName]
  if (!parts) {
    console.warn(`No parts available for view: ${viewName}`)
    return ''
  }
  
  // Check if we have extracted wearables from previewSideAavegotchi for this view
  const extractedWearables = extractedWearablesByView.value[viewName]
  const hasExtractedWearables = extractedWearables && Object.keys(extractedWearables).length > 0
  
  // Check if gotchi has a body wearable (slot 0)
  const hasBodyWearable = (previewWearables.value[0] && previewWearables.value[0] !== 0) || 
                          (gotchiData.value?.equippedWearables?.[0] && gotchiData.value.equippedWearables[0] !== 0)
  ...
  const wearablePositions = {
    0: { // Body
      Front: { x: 7, y: 31, className: 'wearable-body' },
      Left: { x: 20, y: 28, className: 'wearable-body' },
      Right: { x: 20, y: 28, className: 'wearable-body' },
      Back: { x: 20, y: 28, className: 'wearable-body' }
    },
    4: { // Left Hand
      Front: { x: 3, y: 32, className: 'wearable-hand wearable-hand-left' },
      Left: { x: 3, y: 32, className: 'wearable-hand wearable-hand-left' },
    ...
    7: { // Background
      ...
      Right: { x: 0, y: 0, className: 'wearable-bg' },
      Back: { x: 0, y: 0, className: 'wearable-bg' }
    }
  }
  
  // Helper function to wrap wearable content with positioning if needed
  function wrapWearableWithPositioning(content, slot, view) {
    const position = wearablePositions[slot]?.[view]
    if (!position) {
      // No position defined for this slot/view, return as-is
      return content
    }
  ...
  function extractSleevesFromWearable(wearableSvgString, slot, view) {
    if (!wearableSvgString || slot !== 0) return '' // Only extract sleeves from body wearables (slot 0)
    
    try {
      const parser = new DOMParser()
      const doc = parser.parseFromString(wearableSvgString, 'image/svg+xml')
      const svgElement = doc.querySelector('svg')
      if (!svgElement) return ''
      ...
      for (const element of allElements) {
        const classes = (element.getAttribute('class') || '').toLowerCase()
        if (classes.includes('gotchi-sleeves') || classes.includes('sleeve')) {
          // Check if this is sleeves-down (we want to show these when hands are down open)
          ...
          // When body wearable is equipped, we show hands down open, so show sleeves-down
          if (hasBodyWearable) {
            if (isSleevesDown) {
              sleeveElements.push(element.cloneNode(true))
            }
            // Skip sleeves-up when hands are down open
          } else {
            // If no body wearable, include all sleeves
            sleeveElements.push(element.cloneNode(true))
          }
        }
      }
      ...
      // Serialize sleeve elements
      const serializer = new XMLSerializer()
      let sleevesContent = ''
      sleeveElements.forEach(el => {
        sleevesContent += serializer.serializeToString(el)
      })
      
      // Wrap sleeves with positioning (use same position as body wearable)
      const position = wearablePositions[0]?.[view]
      if (position) {
        // Check if sleeves are already wrapped in an SVG with positioning
        // If not, wrap them
        const tempParser = new DOMParser()
        try {
          const tempDoc = tempParser.parseFromString(`<div>${sleevesContent}</div>`, 'text/html')
          const hasNestedSvg = tempDoc.querySelector('svg[x], svg[y]')
          
          if (hasNestedSvg) {
            // Already has positioning, return as-is with wrapper group
            return `<g class="gotchi-wearable wearable-sleeves">${sleevesContent}</g>`
          }
        } catch (e) {
          // If parsing fails, continue with wrapping
        }
        
        return `<g class="gotchi-wearable wearable-sleeves"><svg x="${position.x}" y="${position.y}">${sleevesContent}</svg></g>`
      }
      
      return `<g class="gotchi-wearable wearable-sleeves">${sleevesContent}</g>`
    } catch (error) {
      console.warn(`Failed to extract sleeves from wearable:`, error)
      return ''
    }
  }
  
  const viewBox = '0 0 64 64'
  const partsOrder = ['Background', 'Body', 'Mouth', 'Eyes', 'Collateral', 'Hands']
  
  // Extract inner content from each part SVG
  const partsContent = []
  partsOrder.forEach(partName => {
    if (parts[partName]) {
      const parser = new DOMParser()
      const doc = parser.parseFromString(parts[partName], 'image/svg+xml')
      const svgElement = doc.querySelector('svg')
      if (svgElement) {
        // Get all children of the SVG element
        const children = Array.from(svgElement.children)
        
        // If this is the Hands part and body wearable exists, filter hands appropriately
        if (partName === 'Hands' && hasBodyWearable) {
          children.forEach(child => {
            const className = (child.getAttribute('class') || '').toLowerCase()
            // When body wearable is equipped:
            // - Show gotchi-handsDownOpen (display: block)
            // - Hide gotchi-handsDownClosed (display: none)
            // - Hide gotchi-handsUp (display: none)
            if (className.includes('gotchi-handsdownopen')) {
              // Show hands down open
              partsContent.push(new XMLSerializer().serializeToString(child))
            } else if (className.includes('gotchi-handsdownclosed') || 
                       className.includes('gotchi-handsup')) {
              // Hide hands down closed and hands up
              // Don't add these to partsContent
            } else {
              // Other hand-related elements (like sleeves), include them
              partsContent.push(new XMLSerializer().serializeToString(child))
            }
          })
        } else {
          // For all other parts, include all children
          children.forEach(child => {
            partsContent.push(new XMLSerializer().serializeToString(child))
          })
        }
    ...
    if (partName === 'Background') {
      ...
      if (hasExtractedWearables && extractedWearables[7]) {
        // Use extracted wearables (already correctly positioned)
        extractedWearables[7].forEach(wearable => {
          if (!wearable.isSleeve) {
            partsContent.push(wearable.svg)
          }
        })
        ...
        try {
          const parser = new DOMParser()
          const doc = parser.parseFromString(svgMap[bgWearableId], 'image/svg+xml')
          const svgElement = doc.querySelector('svg')
          if (svgElement) {
            ...
            if (nestedSvgs.length > 0) {
              ...
              // No nested SVGs found - extract content and wrap with positioning
              const children = Array.from(svgElement.children)
              let wearableContent = ''
              ...
              const wrappedContent = wrapWearableWithPositioning(wearableContent, 7, viewName)
              partsContent.push(wrappedContent)
            }
          }
        } catch (error) {
          console.warn(`Failed to parse background wearable ${bgWearableId} SVG:`, error)
        }
        }
      }
    }
  ...
  // Add other wearables after Hands (in slot order: 0=Body, 1=Face, 2=Eyes, 3=Head, 4=Left Hand, 5=Right Hand, 6=Pet)
  ...
  wearableSlotsOrder.forEach(slot => {
    ...
    if (hasExtractedWearables && extractedWearables[slot]) {
      // Use extracted wearables (already correctly positioned)
      // First add non-sleeve wearables
      extractedWearables[slot].forEach(wearable => {
        if (!wearable.isSleeve) {
          partsContent.push(wearable.svg)
        }
      ...
      // Then add sleeves for body wearable (slot 0) - sleeves should come after the body but before hands
      if (slot === 0) {
        extractedWearables[slot].forEach(wearable => {
          if (wearable.isSleeve) {
            // Filter sleeves based on body wearable state
      ...
      try {
        ...
        if (svgElement) {
          ...
          if (nestedSvgs.length > 0) {
            ...
            children.forEach(child => {
              if (child.tagName !== 'style') {
                // If this is a body wearable, check if child contains sleeves
                if (slot === 0) {
                  const childClasses = (child.getAttribute('class') || '').toLowerCase()
                  const isSleeveElement = childClasses.includes('gotchi-sleeves') || childClasses.includes('sleeve')
                  
                  // Also check if child contains sleeve elements inside
                  let containsSleeves = false
                  if (!isSleeveElement) {
                    const sleeveElements = child.querySelectorAll('.gotchi-sleeves, [class*="sleeve"]')
                    containsSleeves = sleeveElements.length > 0
                  }
                  
                  // Skip sleeve elements - we'll extract them separately
                  if (!isSleeveElement && !containsSleeves) {
                    partsContent.push(new XMLSerializer().serializeToString(child))
                  }
                } else {
                  partsContent.push(new XMLSerializer().serializeToString(child))
          ...
          // If this is a body wearable (slot 0), extract and add sleeves
          if (slot === 0) {
            const sleevesSvg = extractSleevesFromWearable(svgMap[wearableId], slot, viewName)
            if (sleevesSvg) {
              partsContent.push(sleevesSvg)
            }
          }
        }
      } catch (error) {
        console.warn(`Failed to parse wearable ${wearableId} SVG:`, error)
      }
      }
    }
  })
  ...
  if (parts.Shadow) {
    const parser = new DOMParser()
    const doc = parser.parseFromString(parts.Shadow, 'image/svg+xml')
    const svgElement = doc.querySelector('svg')
    if (svgElement) {
      const children = Array.from(svgElement.children)
      children.forEach(child => {
  ...
  // Combine all parts into a single SVG
  const combinedContent = partsContent.join('\n')
  const fullSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${viewBox}">\n${combinedContent}\n</svg>`
...
.dressing-room-section {
  @apply bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-lg mb-6 transition-colors;
  @apply flex flex-col;
}
...
.panel-header {
  ...
  @apply sticky top-0 z-10 rounded-t-lg;
}
...
.loading-progress-text {
  @apply text-sm text-blue-600 dark:text-blue-400 text-center font-medium;
}
