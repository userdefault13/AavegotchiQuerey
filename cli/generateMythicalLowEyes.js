...
async function main() {
  console.log('=== Generate Mythical Low Eyes for amaave ===\n')
  ...
  // Mythical Low shapes for Haunt 2
  const mythicalLowShapes = [
    { name: 'Mythical-Low_1', traitValue: 0 },
    { name: 'Mythical-Low_2', traitValue: 1 }
  ]
  
  for (const shape of mythicalLowShapes) {
    const shapeDir = path.join(outputDir, `${shape.name}_Range_${shape.traitValue}-${shape.traitValue}`)
    if (!fs.existsSync(shapeDir)) {
      fs.mkdirSync(shapeDir, { recursive: true })
    }
    
    console.log(`Processing ${shape.name} (trait value ${shape.traitValue})...`)
